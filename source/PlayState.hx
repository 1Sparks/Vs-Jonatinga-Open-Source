...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		trace('test2: ' + executeModchart + " - " + Paths.lua(songLowercase + "/modchart"));
		...
		switch (songLowercase)
		{
			...
			case 'uh':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/uh/uhDialogue'));
			case 'dammit':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/dammit/dammitDialogue'));
			case 'just-kidding':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/just-kidding/just-kiddingDialogue'));
		}
				...
				case 'collector':
				    {
			            defaultCamZoom = 0.8;
			            curStage = 'collector';
						var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('john/collector'));
			            add(bg);
					}
				default:
					{
	...
	override function beatHit()
	{
		...
		iconP1.updateHitbox();
		iconP2.updateHitbox();
		if (curBeat == 89 && curSong == 'just-kidding')
		{
			var bruh:FlxSprite = new FlxSprite();
			bruh.loadGraphic(Paths.image('john/mightybanana'));
			bruh.antialiasing = true;
			bruh.active = false;
			bruh.scrollFactor.set();
			bruh.screenCenter();
			add(bruh);
			FlxG.sound.play(Paths.sound('banana'));
		...
		if (curBeat == 120 && curSong == 'just-kidding')
		{
			var bruh:FlxSprite = new FlxSprite();
			bruh.loadGraphic(Paths.image('john/mightybanana'));
			bruh.antialiasing = true;
		...
		if (curBeat == 173 && curSong == 'just-kidding')
		{
			var bruh:FlxSprite = new FlxSprite();
			bruh.loadGraphic(Paths.image('john/mightybanana'));
			bruh.antialiasing = true;
